...
class DataProcessor:
    ...
    def get_testing_batch(self, batch_no):
        ...
        # ID是问题的唯一标号
        # n就是batch_size
        return paragraph, question, answer, ID, context, n
...
def _process_data(path_lst, word_min_count=5, output_file_path=[]):
    ...
    word2id, id2word = build_word2id(word_count, word_min_count)
    with open('data/word2id.obj', 'wb') as f:
        pickle.dump(word2id, f)
    with open('data/id2word.obj', 'wb') as f:
        pickle.dump(id2word, f)
    for one_raw_data, one_output_file_path in zip(raw_data, output_file_path):
        with open(one_output_file_path, 'wb') as f:
            if 'testa' in one_output_file_path:
                one_data = transform_data_to_id(one_raw_data, word2id, is_test=True)
            else:
                one_data = transform_data_to_id(one_raw_data, word2id)
            pickle.dump(one_data, f)
    return len(word2id), word2id

if __name__ == '__main__':
    sgns_path = './data/sgns.wiki.bigram-char'
    data_path = './data/'
    len_dict, word2id = process_data(data_path, threshold=5)

    word2vec_dict = get_word2vec(sgns_path, word2id)
    with open('./data/pre_vector.json', 'w') as f:
        json.dump(word2vec_dict, f)
